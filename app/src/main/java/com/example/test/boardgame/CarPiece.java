...
public class CarPiece extends Piece{
    ...
    public CarPiece(int pieceIndex, String pieceOwner, Context context, Point initialPos, FrameLayout gameBoardLayout, LinearLayout.LayoutParams lpPiece, int boardRow, int boardColumn) {
        super(pieceIndex, pieceOwner, context, initialPos, gameBoardLayout, lpPiece, boardRow, boardColumn);
        if(pieceOwner.equals("host")){
            pieceImage.setImageDrawable(context.getResources().getDrawable(R.drawable.che_red));
        } else{
            pieceImage.setImageDrawable(context.getResources().getDrawable(R.drawable.che));
        }
    }
    ...
    public boolean isMoveValid(MoveData moveData) {
        if(!(moveData.endColumn == moveData.startColumn) && !(moveData.startRow == moveData.endRow) ){
            // rook can only move in a straight line.
            return false;
        }
    ...
    public boolean isKillValid(MoveData moveData, ChineseChessBoard board) {
        int i;
        int[][] pieceInd = board.boardPieceIndex;
